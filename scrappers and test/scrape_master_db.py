            ...
            for route in routes:
                ...
                if stops:
                    for stop in stops:
                        ...
                        # Add API ID if not already known
                        if api_id not in stops_db[street_id]['API_IDs']:
                            stops_db[street_id]['API_IDs'].append(api_id)
                            stop_counter += 1
                        
                        # Add Line ID if not already known
                        if line_display_id not in stops_db[street_id]['Lines']:
                            stops_db[street_id]['Lines'].append(line_display_id)
